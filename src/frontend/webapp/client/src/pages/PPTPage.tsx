...
export function PPTPage() {
  ...
  return <main className="ppt-page">
    ...
    {editing ? <form onSubmit={async e => { e.preventDefault(); if (job) { await act('revise', { spec: draft }); return } setBusy(true); setError(''); try { const created = await pptApi.create(draft); navigate(`/ppt/${created.id}`) } catch (err) { setError(String(err)) } finally { setBusy(false) } }}>
      ...
      <section className="ppt-review"><h2>大纲与风格</h2><p>{job.spec.style}</p>{job.spec.pages.map((p, i) => <article key={i}><h3>{i + 1}. {p.title}</h3><ul>{p.points.map((point, n) => <li key={n}>{point}</li>)}</ul><p>{p.layout}</p>{p.reference_document_ids.map(doc => <figure key={doc}><img src={`/api/ppt/jobs/${job.id}/references/${doc}`} alt={`第 ${i + 1} 页必需原图`} /><figcaption>请确认这张原图用于本页</figcaption></figure>)}</article>)}</section>
