...
store, graph store, artifacts, agent data) across deployments. Redeployment
MUST NOT delete, reinitialize, or migrate-destructively any data volume.
...
Deployment credentials SHALL be read from a credentials file maintained
outside the pipeline's source checkout. Recreating or wiping the source
checkout MUST NOT lose credentials or deployed data.

#### Scenario: Source checkout recreated
- **WHEN** the pipeline's source checkout is deleted and re-cloned
