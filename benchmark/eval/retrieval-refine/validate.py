...
REQUIRED_RESULT_FIELDS = {
    "case_id", "source_types", "ranks", "scores", "citation_ids",
    "phase_ms", "payload_bytes", "outcome",
}
...
def main() -> None:
    ...
    for result in baseline["results"]:
        ...
        assert len(result["source_types"]) == len(result["ranks"]) == len(result["scores"])
        assert result["payload_bytes"] >= 0
    first = canonical_digest({"cases": cases, "baseline": baseline})
    second = canonical_digest({"cases": cases, "baseline": baseline})
    assert first == second
    print(json.dumps({"status": "ok", "cases": len(case_ids), "digest": first}))
